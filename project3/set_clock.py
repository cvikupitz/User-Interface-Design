...
class ClockWindow:
    ...
    def __init__(self):
        ...
        self.root.bind('<k>', self.forward)
        self.root.bind('<space>', self.next_mode)

        # Create buttons to do the same things as j, k, and spacebar
        self.backwardButton = Button(self.root, text = "<<", command = self.backwardAction)
        self.backwardButton.grid(row = 0, column = 5)
        self.setButton = Button(self.root, text = "Set", command = self.setAction)
        self.setButton.grid(row = 0, column = 6)
        self.forwardButton = Button(self.root, text = ">>", command = self.forwardAction)
        self.forwardButton.grid(row = 0, column = 7)

        # Display the window
        self.root.mainloop()

    # Registers the methods for the buttons
    # The methods registered with the keys do not work with the buttons
    def backwardAction(self):
        self.backward(None)     # Same as backward()
    def setAction(self):
        self.next_mode(None)    # Same as forward()
    def forwardAction(self):
        self.forward(None)      # Same as next_mode()
    ...
    def play_current_hour(self):
        if (self._hour >= 0 and self._hour <= 11):
            sound.Play(self.TIME_WAV_DIR + str(self._hour + 1) + ".wav")
    ...
    def play_current_minute(self):
        ...
        # Minute selected is 1-9, play 'oh' followed by the minute
            ...
            sound.combine_wav_files(self.TMP_FILE_WAV, self.OH_WAV, self.TIME_WAV_DIR + str(self._minute * 5) + ".wav")
            sound.Play(self.TMP_FILE_WAV)
        # Minute selected is 11-59, simply play corresponding file
        else:
            sound.Play(self.TIME_WAV_DIR + str(self._minute * 5) + ".wav")
